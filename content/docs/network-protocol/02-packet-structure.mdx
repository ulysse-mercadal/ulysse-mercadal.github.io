...
- Each **instruction** is self-contained and consists of a **2-byte header** + an optional **body** (0 to 510 bytes).
...
| 13–9     | Instruction ID  | 5-bit unsigned integer (0–31) identifying the command type |
...
- `I` = Instruction ID (5 bits → max 31)
...
| **UDP**  | Real-time gameplay (inputs, state updates)  | Each datagram contains **one full packet** (≤512 bytes). Multiple instructions may be batched, but the entire datagram must fit in one UDP payload. |
...
An `EXAMPLE` instruction (ID = 10) with a 4-byte body sent with direction code `01`:
- Direction = `01`
- ID = `10` → binary `01010`
