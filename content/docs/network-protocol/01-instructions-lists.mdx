...
| 0x0E  | `MOVE`             | `UDP`| Updates a player’s position in 3D space (`x, y, z`). | `yes` |
| 0x0F  | `SET_POSITION`     | `UDP`| Forces an absolute position update. | `yes` |
| 0x10  | `GET_COORDS`       | `UDP`| Requests current coordinates of entities. | `no` |
...
| 0x13  | `SCORE`            | `UDP`| Updates a player’s score. | `no`
| 0x14  | `MESSAGE`          | `UDP`| Sends a text message | `yes`
