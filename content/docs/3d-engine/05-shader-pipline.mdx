...
These method are parts of the **`ShaderController`**, accessible via `Renderer3D::getShaderController()`.
...
```cpp
renderer.getShaderController().setCurrentShader("phong");
```
...
```cpp
auto current = renderer.getShaderController().getCurrentShader();
if (current) {
...
```cpp
auto pixelShader = renderer.getShaderController().getShader("pixelize");
if () {
...
```cpp
auto pixelShader = renderer.getShaderController().getShader("pixelize");
renderer.getShaderController().addShaderToPipeline(pixelShader);
```
...
std::vector<std::shared_ptr<IShader>> pipeline = {
    renderer.getShaderController().getShader("phong"),
    renderer.getShaderController().getShader("pixelize")
};
renderer.getShaderController().setShaderPipeline(pipeline);
```
...
```cpp
const auto& pipeline = renderer.getShaderController().getShaderPipeline();
...
```cpp
renderer.getShaderController().removeShaderFromPipeline("pixelize"); // disables pixelation
```
...
renderer.getShaderController().setShaderPipeline({
    renderer.getShaderController().getShader("phong"),
    renderer.getShaderController().getShader("pixelize")
});
```
...
if (sf::Keyboard::isKeyPressed(sf::Keyboard::P)) {
    if (postEnabled) {
        renderer.getShaderController().clearShaderPipeline();
    } else {
        renderer.getShaderController().setShaderPipeline({
            renderer.getShaderController().getShader("phong"),
            renderer.getShaderController().getShader("pixelize")
...
> The `"phong"` shader simulate a simple phong effect and adapt geometry to the animation, it should be your first shader in a final pipline.
